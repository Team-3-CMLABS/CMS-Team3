...
import { FiX, FiType, FiImage, FiHash, FiClock, FiMapPin, FiLayers, FiLink } from "react-icons/fi";
...
export default function AddFieldModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-2xl shadow-2xl w-[640px] max-w-[95%] p-6 relative">
        <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-slate-700">
          <FiX size={20} />
        </button>

        <h3 className="text-lg font-semibold text-slate-900 text-center mb-4">Add Field Type</h3>
